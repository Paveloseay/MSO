...
public class Sandyruins1Structure {
	...
	private static class FeatureRegisterHandler {
		...
		public static void registerFeature(RegistryEvent.Register<Feature<?>> event) {
				...
				public boolean generate(ISeedReader world, ChunkGenerator generator, Random random, BlockPos pos, NoFeatureConfig config) {
					...
					if ((random.nextInt(1000000) + 1) <= 100000) {
						...
						for (int a = 0; a < count; a++) {
							...
							if (blockAt.getBlock() == GlazesandBlock.block)
								blockCriteria = true;
							if (!blockCriteria)
								continue;
							...
							Template template = world.getWorld().getStructureTemplateManager()
									.getTemplateDefaulted(new ResourceLocation("mso", "sandyruins1"));
							if (template == null)
								return false;
			...
			event.getRegistry().register(feature.setRegistryName("sandyruins_1"));
			Registry.register(WorldGenRegistries.CONFIGURED_FEATURE, new ResourceLocation("mso:sandyruins_1"), configuredFeature);
		}
	}
